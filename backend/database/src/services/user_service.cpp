...
std::optional<user>
user_service::login(const std::string &email, const std::string &password) {
...
bool user_service::verify_password(
    const std::string &password,
...
bool user_service::is_password_strong(const std::string &password) {
    if (password.size() < 8) {
        std::cerr << "[SERVICE]: password too short (less then 8 symbols)\n";
        return false;
    }
